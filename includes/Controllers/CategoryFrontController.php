...
class CategoryFrontController {
	...
	public function list( WP_REST_Request $request ) {
		...
		try {
			...
			$categoriesData = Category::where( 'user_id', $user_id )->pluck('cat_name', 'id');
	...
	public function getCategory( WP_REST_Request $request ) {
		...
		try {
			...
			$category = Category::with('user')->where( 'id', $requestData['id'] )->where('user_id', $user_id )->first();
	...
	public function save( WP_REST_Request $request ) {
		...
		try {
			...
			if ( isset( $data['id'] ) ) {		
				...
				$cat = Category::where( 'id', $data['id'] )->where('user_id', $login_user_id )->first();

				if(!$cat){
					$response->set_data( [
						'success' => false,
						'info'    => esc_html__( 'Category not found', 'cbxwpbookmark' )
					] );
...
} //end class CategoryFrontController