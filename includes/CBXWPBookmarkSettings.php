...
class CBXWPBookmarkSettings {
	...
	function callback_multicheck( $args, $value = null ) {
		...
		$html = '<p class="grouped gapless grouped_buttons checkbox_fields_check_actions"><a href="#" class="button primary checkbox_fields_check_action_call">' . esc_html__( 'Check All', 'cbxwpbookmark' ) . '</a><a href="#" class="button outline checkbox_fields_check_action_ucall">' . esc_html__( 'Uncheck All', 'cbxwpbookmark' ) . '</a></p>';
	...
	function callback_page( $args ) {
		$edit_svg     = cbxwpbookmark_esc_svg( cbxwpbookmark_load_svg( 'icon_edit' ) );
		$external_svg = cbxwpbookmark_esc_svg( cbxwpbookmark_load_svg( 'icon_external' ) );
		...
		if ( $value > 0 ) {
			if ( $check_content != '' ) {
				...
				if ($page !== null && has_shortcode( $page->post_content, $check_content ) ) {
					...
					$page_shortcode_note   = sprintf( esc_html__( 'This page has shortcode %s', 'cbxwpbookmark' ), $check_content );
				} else {
					$shortcode_found_class = 'description_note_off';
					/* translators: %s: Shortcode name  */
					$page_shortcode_note   = sprintf( esc_html__( 'This page doesn\'t have shortcode %s', 'cbxwpbookmark' ), $check_content );
				}
			}

			//edit
			if ( current_user_can( 'edit_post', $value ) ) {
				$page_html .= '<a class="setting_pages_action setting_pages_action_edit button primary icon icon-only icon-inline small" target="_blank" title="' . esc_attr__( 'Edit', 'cbxwpbookmark' ) . '" href="' . esc_url( get_edit_post_link( $value ) ) . '"><i class="cbx-icon">'.$edit_svg.'</i></a>';
			}

			//view
			$page_html .= '<a class="setting_pages_action setting_pages_action_view button outline primary icon icon-only icon-inline small" title="' . esc_attr__( 'View', 'cbxwpbookmark' ) . '" target="_blank" href="' . esc_url( get_the_permalink( $value ) ) . '"><i class="cbx-icon">'.$external_svg.'</i></a>';
		}
		...
		if ( $args['placeholder'] == '' ) {
			$placeholder = $args['placeholder'] = esc_html__( 'Please Select', 'cbxwpbookmark' );
		} else {
	...
	function callback_color( $args, $value = null ) {
		...
		$choose_color = esc_html__( 'Choose Color', 'cbxwpbookmark' );
			...
			<div class="metabox-holder">
				...
				foreach ( $this->settings_sections as $form ) {
					...
					<div id="<?php echo esc_attr( $form['id'] ); ?>" class="global_setting_group" style="<?php echo esc_attr( $display_style ); ?>">
						<form method="post" action="options.php" class="cbxwpbookmark_setting_form">
							<?php
							do_action( 'cbxwpbookmark_setting_form_start', $form );
							do_action( 'cbxwpbookmark_setting_form_top_' . $form['id'], $form );

							settings_fields( $form['id'] );
							do_settings_sections( $form['id'] );

							do_action( 'cbxwpbookmark_setting_form_bottom_' . $form['id'], $form );
							do_action( 'cbxwpbookmark_setting_form_end', $form );
							?>

							<div class="global_setting_submit_buttons_wrap">
								<?php do_action( 'cbxwpbookmark_setting_submit_buttons_start', $form['id'] ); ?>
								<?php submit_button( esc_html__( 'Save Settings', 'cbxwpbookmark' ), 'button primary submit_setting', 'submit', true, [ 'id' => 'submit_' . esc_attr( $form['id'] ) ] ); ?>
								<?php do_action( 'cbxwpbookmark_setting_submit_buttons_end', $form['id'] ); ?>
							</div>
						</form>
					</div>
					<?php
