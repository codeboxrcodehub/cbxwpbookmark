...
class CBXWPBookmarkPublic {
	...
	public function init_widgets() {
		...
		require_once CBXWPBOOKMARK_ROOT_PATH . 'includes/Widgets/Classic/Category/CBXWPBookmarkCategoryWidget.php';
		require_once CBXWPBOOKMARK_ROOT_PATH . 'includes/Widgets/Classic/Most/CBXWPBookmarkMostWidget.php';
		require_once CBXWPBOOKMARK_ROOT_PATH . 'includes/Widgets/Classic/MyBookmark/CBXWPBookmarkMyBookmarkWidget.php';
	...
	public function init_elementor_widgets() {
		...
		\Elementor\Plugin::instance()->widgets_manager->register( new CBXWPBookmarkBtnElemWidget() );


		//register the my bookmarks widget
		\Elementor\Plugin::instance()->widgets_manager->register( new CBXWPBookmarkMyBookmarkElemWidget() );


		//register the bookmark category widget
		\Elementor\Plugin::instance()->widgets_manager->register( new CBXWPBookmarkCategoryElemWidget() );


		//register the most bookmarked posts widget
		\Elementor\Plugin::instance()->widgets_manager->register( new CBXWPBookmarkMostElemWidget() );
	...
	public function vc_before_init_actions() {
		//include vc params
		new CBXWPBookmarkVCParamDropDownMulti();

		//includes the vc widgets
		//bookmark button widget
		new CBXWPBookmarkBtnVCWidget();

		//my bookmarked post widget
		new CBXWPBookmarkMyBookmarkVCWidget();

		//my bookmark category widget
		new CBXWPBookmarkCategoryVCWidget();


		//most bookmarked post widget
		new CBXWPBookmarkMostVCWidget();
	...
	/**
	 * User dashboard overview dashboard menu
	 *
	 * @param $menus
	 *
	 * @return array|mixed
	 */
	public function user_dashboard_dashboard_menu( $menus ) {
		if ( is_array( $menus ) ) {
			$menus['user_dashboard'] = [
				'title'    => esc_html__( 'Bookmarks Overview', 'cbxwpbookmark' ),
				'type'     => 'callback', //shortcode, callback
				'callback' => [ $this, 'user_dashboard_menu_page' ]
			];
		}

		return $menus;
	} //end method user_dashboard_dashboard_menu
	...
	public function user_dashboard_menu_page() {
		...
		if ( defined( 'CBXWPBOOKMARK_DEV_MODE' ) && CBXWPBOOKMARK_DEV_MODE == true ) {
			...
			wp_register_script( 'cbxwpbookmark_dashboard_vue_dev', 'http://localhost:8880/assets/vuejs/apps/front/dashboard.js', [], $ver, $t );
			...
			// for production
			wp_register_script( 'cbxwpbookmark_dashboard_vue_main', $js_url_part_build . 'frontdashboard.js', [], $ver, $t );
		...
		return "<div id='cbxwpbookmark-front-stat-dashboard'></div>";
	} //end method user_dashboard_menu_page

	/**
	 * User dashboard category menu
	 *
	 * @param $menus
	 *
	 * @return array|mixed
	 */
	public function user_dashboard_category_menu( $menus ) {
		$settings      = $this->settings;
		$bookmark_mode = $settings->get_field( 'bookmark_mode', 'cbxwpbookmark_basics', 'user_cat' );

		if ( $bookmark_mode == 'user_cat' ) {
			if ( is_array( $menus ) ) {
				$menus['category_manager'] = [
					'title'    => esc_html__( 'Category Manager', 'cbxwpbookmark' ),
					'type'     => 'callback', //shortcode, callback
					'callback' => [ $this, 'category_dashboard_page' ]
				];
			}
		}

		return $menus;
	} //end method user_dashboard_category_menu
	...
	public function category_dashboard_page() {
		...
		if ( defined( 'CBXWPBOOKMARK_DEV_MODE' ) && CBXWPBOOKMARK_DEV_MODE == true ) {
			//for development version
			wp_register_script( 'cbxwpbookmark_category_vue_dev', 'http://localhost:8880/assets/vuejs/apps/front/category.js', [], $ver, $t );
			wp_localize_script( 'cbxwpbookmark_category_vue_dev', 'cbxwpbookmark_vue_var', $js_translations );
			wp_enqueue_script( 'cbxwpbookmark_category_vue_dev' );
		} else {
			// for production
			wp_register_script( 'cbxwpbookmark_category_vue_main', $js_url_part_build . 'frontcategory.js', [], $ver, $t );
			wp_localize_script( 'cbxwpbookmark_category_vue_main', 'cbxwpbookmark_vue_var', $js_translations );
			wp_enqueue_script( 'cbxwpbookmark_category_vue_main' );
		}

		return "<div id='cbxwpbookmark-front-category-manager'></div>";
	} //end method category_dashboard_page

	/**
	 * User dashboard bookmark menu
	 *
	 * @param $menus
	 *
	 * @return array|mixed
	 */
	public function user_dashboard_bookmarks_menu( $menus ) {

		if ( is_array( $menus ) ) {
			$menus['bookmark_manager'] = [
				'title'    => esc_html__( 'Bookmark Manager', 'cbxwpbookmark' ),
				'type'     => 'callback', //shortcode, callback
				'callback' => [ $this, 'bookmark_dashboard_page' ]
			];
		}
	...
	public function bookmark_dashboard_page() {
		...
		if ( defined( 'CBXWPBOOKMARK_DEV_MODE' ) && CBXWPBOOKMARK_DEV_MODE == true ) {
			//for development version
			wp_register_script( 'cbxwpbookmark_vue_dev', 'http://localhost:8880/assets/vuejs/apps/front/bookmark.js', [], $ver, $t );
			wp_localize_script( 'cbxwpbookmark_vue_dev', 'cbxwpbookmark_vue_var', $js_translations );
			wp_enqueue_script( 'cbxwpbookmark_vue_dev' );
		} else {
			// for production
			wp_register_script( 'cbxwpbookmark_vue_main', $js_url_part_build . 'frontbookmark.js', [], $ver, $t );
			wp_localize_script( 'cbxwpbookmark_vue_main', 'cbxwpbookmark_vue_var', $js_translations );
			wp_enqueue_script( 'cbxwpbookmark_vue_main' );
		}

		return "<div id='cbxwpbookmark-front-bookmark-manager'></div>";
	} //end method bookmark_dashboard_page
}//end class CBXWPBookmarkPublic