...
namespace CBXWPBookmarkScoped\Sabberworm\CSS\Tests;
...
use CBXWPBookmarkScoped\Sabberworm\CSS\Parser;
...
use CBXWPBookmarkScoped\Sabberworm\CSS\Property\Charset;
use CBXWPBookmarkScoped\Sabberworm\CSS\Property\CSSNamespace;
use CBXWPBookmarkScoped\Sabberworm\CSS\Property\Import;
use CBXWPBookmarkScoped\Sabberworm\CSS\Property\Selector;
use CBXWPBookmarkScoped\Sabberworm\CSS\RuleSet\AtRuleSet;
...
use CBXWPBookmarkScoped\Sabberworm\CSS\Tests\RuleSet\DeclarationBlockTest;
use CBXWPBookmarkScoped\Sabberworm\CSS\Value\Color;
use CBXWPBookmarkScoped\Sabberworm\CSS\Value\Size;
use CBXWPBookmarkScoped\Sabberworm\CSS\Value\URL;
/**
...
final class ParserTest extends TestCase
{
    ...
    public function colorParsing()
    {
        ...
        foreach ($oDoc->getAllRuleSets() as $oRuleSet) {
            ...
            if ($sSelector === '#mine') {
                ...
                $oColor = $aColorRule[0]->getValue();
                self::assertEquals(['r' => new Size(35.0, null, \true, $oColor->getLineNo()), 'g' => new Size(35.0, null, \true, $oColor->getLineNo()), 'b' => new Size(35.0, null, \true, $oColor->getLineNo())], $oColor->getColor());
                $aColorRule = $oRuleSet->getRules('border-color');
                $oColor = $aColorRule[0]->getValue();
                self::assertEquals(['r' => new Size(10.0, null, \true, $oColor->getLineNo()), 'g' => new Size(100.0, null, \true, $oColor->getLineNo()), 'b' => new Size(230.0, null, \true, $oColor->getLineNo())], $oColor->getColor());
                $oColor = $aColorRule[1]->getValue();
                self::assertEquals(['r' => new Size(10.0, null, \true, $oColor->getLineNo()), 'g' => new Size(100.0, null, \true, $oColor->getLineNo()), 'b' => new Size(231.0, null, \true, $oColor->getLineNo()), 'a' => new Size("0000.3", null, \true, $oColor->getLineNo())], $oColor->getColor());
                $aColorRule = $oRuleSet->getRules('outline-color');
                $oColor = $aColorRule[0]->getValue();
                self::assertEquals(['r' => new Size(34.0, null, \true, $oColor->getLineNo()), 'g' => new Size(34.0, null, \true, $oColor->getLineNo()), 'b' => new Size(34.0, null, \true, $oColor->getLineNo())], $oColor->getColor());
            } elseif ($sSelector === '#yours') {
                $aColorRule = $oRuleSet->getRules('background-color');
                $oColor = $aColorRule[0]->getValue();
                self::assertEquals(['h' => new Size(220.0, null, \true, $oColor->getLineNo()), 's' => new Size(10.0, '%', \true, $oColor->getLineNo()), 'l' => new Size(220.0, '%', \true, $oColor->getLineNo())], $oColor->getColor());
                $oColor = $aColorRule[1]->getValue();
                self::assertEquals(['h' => new Size(220.0, null, \true, $oColor->getLineNo()), 's' => new Size(10.0, '%', \true, $oColor->getLineNo()), 'l' => new Size(220.0, '%', \true, $oColor->getLineNo()), 'a' => new Size(0.3, null, \true, $oColor->getLineNo())], $oColor->getColor());
                $aColorRule = $oRuleSet->getRules('outline-color');
    ...
    public function specificity()
    {
        ...
        self::assertEquals([new Selector('#test .help', \true)], $oDoc->getSelectorsBySpecificity('> 100'));
        self::assertEquals([new Selector('#test .help', \true), new Selector('#file', \true)], $oDoc->getSelectorsBySpecificity('>= 100'));
        self::assertEquals([new Selector('#file', \true)], $oDoc->getSelectorsBySpecificity('=== 100'));
        self::assertEquals([new Selector('#file', \true)], $oDoc->getSelectorsBySpecificity('== 100'));
        self::assertEquals([new Selector('#file', \true), new Selector('.help:hover', \true), new Selector('li.green', \true), new Selector('ol li::before', \true)], $oDoc->getSelectorsBySpecificity('<= 100'));
        self::assertEquals([new Selector('.help:hover', \true), new Selector('li.green', \true), new Selector('ol li::before', \true)], $oDoc->getSelectorsBySpecificity('< 100'));
        self::assertEquals([new Selector('li.green', \true)], $oDoc->getSelectorsBySpecificity('11'));
        self::assertEquals([new Selector('ol li::before', \true)], $oDoc->getSelectorsBySpecificity('3'));
    }
    /**
     * @test
     */
    public function manipulation()
    {
        ...
        self::assertSame('@charset "utf-8";' . "\n" . '@font-face {font-family: "CrassRoots";src: url("../media/cr.ttf");}' . "\n" . '#my_id html, #my_id body {font-size: -.6em;}' . "\n" . '@keyframes mymove {from {top: 0px;}' . "\n\t" . 'to {top: 200px;}}' . "\n" . '@-moz-keyframes some-move {from {top: 0px;}' . "\n\t" . 'to {top: 200px;}}' . "\n" . '@supports ( (perspective: 10px) or (-moz-perspective: 10px) or (-webkit-perspective: 10px) ' . 'or (-ms-perspective: 10px) or (-o-perspective: 10px) ) {#my_id body {font-family: "Helvetica";}}' . "\n" . '@page :pseudo-class {margin: 2in;}' . "\n" . '@-moz-document url(https://www.w3.org/),' . "\n" . '               url-prefix(https://www.w3.org/Style/),' . "\n" . '               domain(mozilla.org),' . "\n" . '               regexp("https:.*") {#my_id body {color: purple;background: yellow;}}' . "\n" . '@media screen and (orientation: landscape) {@-ms-viewport {width: 1024px;height: 768px;}}' . "\n" . '@region-style #intro {#my_id p {color: blue;}}', $oDoc->render(OutputFormat::create()->setRenderComments(\false)));
        $oDoc = self::parsedStructureForFile('values');
        self::assertSame('#header {margin: 10px 2em 1cm 2%;font-family: Verdana,Helvetica,"Gill Sans",sans-serif;' . 'font-size: 10px;color: red !important;background-color: green;' . 'background-color: rgba(0,128,0,.7);frequency: 30Hz;transform: rotate(1turn);}
body {color: green;font: 75% "Lucida Grande","Trebuchet MS",Verdana,sans-serif;}', $oDoc->render());
        foreach ($oDoc->getAllRuleSets() as $oRuleSet) {
            $oRuleSet->removeRule('font-');
        }
        self::assertSame('#header {margin: 10px 2em 1cm 2%;color: red !important;background-color: green;' . 'background-color: rgba(0,128,0,.7);frequency: 30Hz;transform: rotate(1turn);}
body {color: green;}', $oDoc->render());
        foreach ($oDoc->getAllRuleSets() as $oRuleSet) {
            $oRuleSet->removeRule('background-');
        }
        self::assertSame('#header {margin: 10px 2em 1cm 2%;color: red !important;frequency: 30Hz;transform: rotate(1turn);}
body {color: green;}', $oDoc->render());
    }
    /**
     * @test
    ...
    public function slashedValues()
    {
        ...
        self::assertSame('.test {font: 12px/1.5 Verdana,Arial,sans-serif;border-radius: 5px 10px 5px 10px/10px 5px 10px 5px;}', $oDoc->render());
        ...
        self::assertSame('.test {font: 36px/1.5 Verdana,Arial,sans-serif;border-radius: 15px 30px 15px 30px/30px 15px 30px 15px;}', $oDoc->render());
    }
    /**
     * @test
     */
    public function functionSyntax()
    {
        ...
        self::assertSame($sExpected, $oDoc->render());
        foreach ($oDoc->getAllValues(null, \true) as $mValue) {
...
|test {gaga: 2;}';
    ...
    public function listValueRemoval()
    {
        ...
        self::assertSame('html {some-other: -test(val1);}
@media screen {html {some: -test(val2);}}
#unrelated {other: yes;}', $oDoc->render());
        $oDoc = self::parsedStructureForFile('nested');
        foreach ($oDoc->getAllDeclarationBlocks() as $oBlock) {
            $oDoc->removeDeclarationBlockBySelector($oBlock, \true);
            break;
        }
        self::assertSame('@media screen {html {some: -test(val2);}}
#unrelated {other: yes;}', $oDoc->render());
    }
    /**
     * @test
...
EXPECTED;
    ...
    {
        $oDoc = self::parsedStructureForFile('url', Settings::create()->withMultibyteSupport(\true));
...
body {background-url: url("https://somesite.com/images/someimage.gif");}';
    ...
    {
        $oDoc = self::parsedStructureForFile('hex-alpha', Settings::create()->withMultibyteSupport(\true));
...
div {background: rgba(17,34,51,.27);}';
    ...
    {
        $oDoc = self::parsedStructureForFile('calc', Settings::create()->withMultibyteSupport(\true));
...
div {width: calc(50% - ( ( 4% ) * .5 ));}';
    ...
    {
        $oDoc = self::parsedStructureForFile('calc-nested', Settings::create()->withMultibyteSupport(\true));
    ...
    {
        $oDoc = self::parsedStructureForFile('calc-invalid', Settings::create()->withMultibyteSupport(\true));
        $sExpected = 'div {}
div {}
div {}
div {height: -moz-calc;}
div {height: calc;}';
    ...
    {
        $oDoc = self::parsedStructureForFile('grid-linename', Settings::create()->withMultibyteSupport(\true));
    ...
    {
        $oDoc = self::parsedStructureForFile('invalid-grid-linename', Settings::create()->withMultibyteSupport(\true));
        $sExpected = "div {}";
        self::assertSame($sExpected, $oDoc->render());
    }
    /**
    ...
    {
        $oDoc = self::parsedStructureForFile('unmatched_braces', Settings::create()->withMultibyteSupport(\true));
    ...
    {
        $oDoc = self::parsedStructureForFile('invalid-selectors', Settings::create()->withMultibyteSupport(\true));
        $sExpected = '@keyframes mymove {from {top: 0px;}}
#test {color: white;background: green;}
#test {display: block;background: white;color: black;}';
        self::assertSame($sExpected, $oDoc->render());
        $oDoc = self::parsedStructureForFile('invalid-selectors-2', Settings::create()->withMultibyteSupport(\true));
...
body {background-color: red;}';
    ...
    {
        $oDoc = self::parsedStructureForFile('selector-escapes', Settings::create()->withMultibyteSupport(\true));
        $sExpected = '#\# {color: red;}
.col-sm-1\/5 {width: 20%;}';
        self::assertSame($sExpected, $oDoc->render());
        $oDoc = self::parsedStructureForFile('invalid-selectors-2', Settings::create()->withMultibyteSupport(\true));
...
body {background-color: red;}';
    ...
    {
        $oDoc = self::parsedStructureForFile('identifier-escapes', Settings::create()->withMultibyteSupport(\true));
    ...
    {
        $oDoc = self::parsedStructureForFile('selector-ignores', Settings::create()->withMultibyteSupport(\true));
    ...
    {
        $oDoc = self::parsedStructureForFile('keyframe-selector-validation', Settings::create()->withMultibyteSupport(\true));
    ...
    {
        $oDoc = self::parsedStructureForFile('-empty', Settings::create()->withMultibyteSupport(\true));
        $sExpected = '';
        self::assertSame($sExpected, $oDoc->render());
    }
    /**
    ...
    {
        $oDoc = self::parsedStructureForFile('-charset-after-rule', Settings::create()->withLenientParsing(\true));
    ...
    {
        $oDoc = self::parsedStructureForFile('-charset-in-block', Settings::create()->withLenientParsing(\true));
    ...
    {
        $parsed = self::parsedStructureForFile('missing-property-value', Settings::create()->withLenientParsing(\true));
    ...
    {
        $oDoc = self::parsedStructureForFile('ie-hacks', Settings::create()->withLenientParsing(\true));
    ...
    public function escapedSpecialCaseTokens()
    {
        ...
        $urlRule = $rules[0];
        $calcRule = $rules[1];
        self::assertTrue(is_a($urlRule->getValue(), 'CBXWPBookmarkScoped\Sabberworm\CSS\Value\URL'));
        self::assertTrue(is_a($calcRule->getValue(), 'CBXWPBookmarkScoped\Sabberworm\CSS\Value\CalcFunction'));
    }
}
