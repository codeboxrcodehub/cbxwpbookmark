...
use Closure;
use CBXWPBookmarkScoped\Rakit\Validation\Rules\Interfaces\BeforeValidate;
use CBXWPBookmarkScoped\Rakit\Validation\Rules\Interfaces\ModifyValue;
use CBXWPBookmarkScoped\Rakit\Validation\Rules\Required;
class Validation
{
    ...
    protected function resolveRules($rules): array
    {
        ...
        foreach ($rules as $i => $rule) {
            ...
            if (is_string($rule)) {
                ...
                $validator = call_user_func_array($validatorFactory, ['CBXWPBookmarkScoped\callback', $rule]);
            } else {
